...
def SyntacticAnalysis(tokens: list[Token], is_nested=False):
    ...
    while read_head < len(tokens) and not hasErrored:
        ...
        if tokens[read_head].type == 'NAME' and read_head + 1 < len(tokens) and tokens[read_head + 1].value == '(':
            fn_name = tokens[read_head]
            args = []
            read_head += 2
            
            while tokens[read_head].value != ')' and read_head < tokens.__len__():
                args.append(tokens[read_head])
                read_head += 1
                
            args = SyntacticAnalysis(args)
            operations.append({'type': 'CallExperession', 'line': fn_name.line, 'args': args, 'name': fn_name.value})
            read_head += 1
            continue
        ...
        if token.type == 'CONSTRUCT':
            ...
            if construct_name.value != 'Shaderbind' and construct_name.value != 'ShareData' and construct_name.value != 'supposedef':
            ...
            if construct_name.value == 'Shaderbind' and argument_0.value != 'VERTEX' and argument_0.value != 'FRAGMENT':
        ...
        if token.type == 'NAME' and token.value == 'var':
            ...
            if (tokens[read_head + 2].type != 'SYMB_COLON'):    
                raise Exception(f'{var_name.line + 1}: Missing type for variable {var_name.value}')
            ...
            read_head += 1
            operation = {'type': 'VariableDeclaration', 'name': var_name.value, 'value': value, 'line': var_name.line, 'vartype': var_type.value}
            operations.append(operation)
            continue
        ...
        if token.type == 'NAME' and token.value == 'fn':
            ...
            # read_head += 1
            operation = {'type': 'FunctionDeclaration', 'name': fn_name.value, 'args': args, 'body': SyntacticAnalysis(contents, True), 'line': fn_name.line}
            operations.append(operation)
            continue
    
        if token.type == 'NAME':
            operation = {'type': 'IdentifierRef', 'name': token.value}
            operations.append(operation)
            read_head += 1
            continue
        print(operations)
